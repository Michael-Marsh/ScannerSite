...
namespace M2kClient
{
    ...
    { 
        ...
        /// <summary>
        /// Get all the information pertaining to a lot number
        /// </summary>
        /// <param name="recordID">Lot Number</param>
        /// <param name="connection">UniConnection to use for the query</param>
        /// <returns>List of lot identity values or a list with ERROR involved</returns>
        public static List<string> GetLotValues(string recordID, M2kConnection connection)
        {
            ...
            using (UniSession uSession = UniObjects.OpenSession(connection.HostName, connection.UserName, connection.Password, connection.UniAccount, connection.UniService))
            {
                try
                {
                    //Extracting the Part ID, if nothing returns then skipping to an error message to return.
                    using (UniFile uFile = uSession.CreateUniFile("LOT.MASTER"))
                    {
                        //0 as Part Number
                        using (UniDynArray udArray = uFile.Read(recordID))
                        {
                            _rtnList.Add(udArray.Extract(1).StringValue);
                        }
                    }
                    if (_rtnList.Count > 0)
                    {
                        ...
                        {
                            using (UniDynArray udArray = uFile.Read(_rtnList[0]))
                            {
                                _rtnList.Add(udArray.Extract(3).StringValue);
                            }
                        }
                        //2 as Location
                        //3 as On Hand
                        using (UniFile uFile = uSession.CreateUniFile("LOT.MASTER"))
                        {
                            using (UniDynArray udArray = uFile.Read(recordID))
                            {
                                _rtnList.Add(udArray.Extract(8).StringValue);
                                _rtnList.Add(udArray.Extract(9).StringValue);
                            }
                        }
                    }
                    else
                    {
                        _rtnList.Add("ERROR:Lot not on file.");
                    }
                    UniObjects.CloseSession(uSession);
                    return _rtnList;
                }
                catch (Exception)
                {
                    ...
                    _rtnList.Clear();
                    _rtnList.Add("ERROR:Record not found.");
                    return _rtnList;
                }
            }
        ...
        /// <summary>
        /// Get the Unit of Measurement for a product ID
        /// </summary>
        /// <param name="recordID">Part Number</param>
        /// <param name="connection">UniConnection to use for the query</param>
        /// <returns>Unit of Measure or empty string on error</returns>
        public static string GetUoM(string recordID, M2kConnection connection)
        {
            recordID += "|01";
            ...
            using (UniSession uSession = UniObjects.OpenSession(connection.HostName, connection.UserName, connection.Password, connection.UniAccount, connection.UniService))
            {
                try
                {
                    //Extracting the UoM, if nothing returns then it skips the entry and returns an empty string.
                    using (UniFile uFile = uSession.CreateUniFile("IM"))
                    {
                        using (UniDynArray udArray = uFile.Read(recordID))
                        {
                            _rtnVal = udArray.Extract(3).StringValue;
                        }
                    }
                    UniObjects.CloseSession(uSession);
                    return _rtnVal;
        ...
        /// <param name="connection">UniConnection to use for the query</param>
        /// <returns>Unit of Measure or empty string on error</returns>
        public static bool LocationExists(string recordID, M2kConnection connection)
        {
            var _rtnVal = false;
            ...
            using (UniSession uSession = UniObjects.OpenSession(connection.HostName, connection.UserName, connection.Password, connection.UniAccount, connection.UniService))
            {
                try
                {
                    using (UniFile uFile = uSession.CreateUniFile("LOC.MASTER"))
                    {
                        using (UniDynArray udArray = uFile.Read(recordID))
                        {
                            _rtnVal = !string.IsNullOrEmpty(udArray.Extract(1).StringValue);
                        }
                    }
                    UniObjects.CloseSession(uSession);
                    return _rtnVal;
